...
fn run2(a_len: usize, a_start: usize, a_end: usize, b_len: usize, b_start: usize, b_end: usize) -> Result<(), Box<dyn Error>> {
...
fn run(a_len: usize, a_start: usize, a_end: usize, b_len: usize, b_start: usize, b_end: usize) -> Result<(), Box<dyn Error>> {
    ...
    let iterator_count = a.symmetric_difference(&b).count();
    let a_sym_diff_b = a.symmetric_difference(&b).collect::<FixedBitSet>();
    println!("count: {:?}, iterator_count: {:?}, a_sym_diff_b: {:?}", count, iterator_count, a_sym_diff_b);
    ...
    let iters = if cfg!(miri) { 24 } else { 64 };
    // let iters = if cfg!(miri) { 48 } else { 128 };
    for i in 1..iters {
        let tmp = FixedBitSet::with_capacity(i);
        for j in 0..tmp.len() + 1 {
            for k in j..tmp.len() + 1 {
                _ = tmp.count_ones(j..k);
            }
        }
    }
...
#[test]
fn quick_test() {
    // let (a_len, a_start, a_end, b_len, b_start, b_end) = (137, 32, 120, 173, 50, 107);
    let (a_len, a_start, a_end, b_len, b_start, b_end) = (421,168,242,1104,145,875);
    let res = run(a_len, a_start, a_end, b_len, b_start, b_end);
    println!("- result: {:?}", res);
}